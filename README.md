...
- **Natural hazards (`env_hazard_overview` / `env_hazard_regions`)**: the former `naturgefahren.ch/api/v1/warnings/*` REST endpoints were **decommissioned (2026, HTTP 301→404)** with no drop-in replacement — the site no longer exposes a public warnings JSON. Both tools therefore **degrade gracefully to curated direct links** to the hazard portal and do not currently return structured live warnings. The authoritative machine-readable source is **MeteoSwiss**, which overlaps `meteoswiss-mcp` (weather warnings) — a repointing there is tracked as a cross-portfolio follow-up. See [`docs/probe-naturgefahren-waldbrand.md`](docs/probe-naturgefahren-waldbrand.md).
